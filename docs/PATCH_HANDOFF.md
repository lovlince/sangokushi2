...
- **현재 Patch 최종 기준본: v260-intermediate (`HexFontEffect_021.png` 단일 이미지 갱신, 정적 검증 완료·Citra 확인 pending)** — exact v259 Patch 84파일을 `analysis\v260_hexfonteffect_021_image_update_baseline\PatchSnapshot`에 봉인하고, 사용자가 명시한 256×64 RGBA PNG만 `RomFS/Hex/UI/HexFontEffect.g1t` index 21에 1:1 재삽입했다. v130 방향 규칙에 따라 추가 반전·회전 없이 표준 RGBA8 encoder에 직접 넣었고 readback은 원 PNG와 pixel-exact, 상하반전 실패형과 불일치한다. 32-texture 구조, header와 비대상 31 payload는 v259 byte-exact다. v259 대비 game diff는 이 G1T 1개뿐, SHA-256=`9F707B770549BFD000E0BDC1832049FA20C7DE7ADA6ADD39C569D02778304DC5`; 나머지 83파일은 v259 byte-exact다. 이미지 생성 없음, builder/독립 verifier/deterministic check PASS. Rebuild·`0004000000174D00`·Dummy update·Backup·패키징은 미수정. report=`analysis\v260_hexfonteffect_021_image_update_report.json`.
- **직전 Patch 최종 기준본: v259-intermediate (`title_up_002.png` + `ktlogo_000.png` 이미지 갱신, 정적 검증 완료·Citra 확인 pending)** — exact v258 Patch 84파일을 `analysis\v259_image_update_baseline\PatchSnapshot`에 봉인하고 명시된 두 PNG만 최신 G1T에 1:1 재삽입했다. `ktlogo_000.png`는 `RomFS/Opening/ktlogo.g1t` index 0, `title_up_002.png`는 `RomFS/StartMenu/title_up.g1t` index 2에 적용했다. 두 readback은 원 PNG와 pixel-exact이며 G1T header, `ktlogo` index 1, 공식 v1.1 `title_up` index 0/1 및 3-texture 구조는 v258 byte-exact다. v258 대비 game diff는 두 G1T뿐이고 SHA-256은 `ktlogo.g1t=699F0F311853C7A83F0595837F6CEE324EA95C18D8057D7ECE23A138C2A5EEF6`, `title_up.g1t=88B0B08BE80B1E466FA53C5A9E719AD03945964ECDBA322E6B8D5B4DE1F58487`; 나머지 82파일은 v258 byte-exact다. 이미지 생성 없음, builder/독립 verifier/deterministic check PASS. Rebuild·`0004000000174D00`·Dummy update·Backup·패키징은 미수정. report=`analysis\v259_image_update_report.json`.
- **직전 Patch 최종 기준본: v258-intermediate (Issue #233/#234 튜토리얼 전투기술 명칭 정리, 정적 검증 완료·Citra 확인 pending)** — exact v257 Patch 84파일을 `analysis\v258_issue233_234_tutorial_label_cleanup_baseline\PatchSnapshot`에 봉인했다. #233은 사용자 최종 지시대로 `msgsec15.dat 0x5F2`의 **`단기진→돌진`만** 7B fixed span에서 적용하고 `반사`와 long `단기돌진`은 byte-exact 보존한다. #234는 exact skill-name **`성벽넘기→벽넘기`**를 current runtime 4곳(`code.bin 0x1D3A10`, `msgsec15 0x5A5`, `0x607`, M15-138 내부 `0x1255`)에 적용했다. `0x607`은 과거 separator-borrow `성벽넘기 + 05 05`에서 `벽넘기 + pad1 + 05 05 05`로 되돌려 다음 record `0x611`을 보존한다. M15-138은 `성벽넘기에 필요한`만 `벽넘기에 필요한`으로 줄이며 자연어 `성벽까지 넘게 해주는 명마`는 그대로다. 실제 전투 compact `code.bin 0x1CEFD4=벽넘기`와 두 pointer는 v257 byte-exact다. v257 대비 game diff는 `ExeFS/code.bin`, `RomFS/Message/msgsec15.dat` 2개뿐이고 SHA-256=`code 204DE93DF01192D92ED5F80476FD00DA3F2DB6825B167ED5967F9CDAAD00F639`, `msgsec15 28C112FA03C75AA09730B45C3D1D139414E2D11C82777966135E69166EB083EE`; 나머지 82파일은 byte-exact다. relocation/file growth/font/PNG/G1T/Scenario/다른 Message 수정 없음. builder/독립 verifier/deterministic check PASS, battle review **v258 authority 520행 PASS**. report=`analysis\v258_issue233_234_tutorial_label_cleanup_report.json`.
- **직전 Patch 최종 기준본: v257-intermediate (Issue #232 특산 아이템 효과 5종 한글화, 정적 검증 완료·Citra runtime PASS)** — exact v256 Patch 84파일을 `analysis\v257_issue232_special_item_effects_baseline\PatchSnapshot`에 봉인하고 `ExeFS/code.bin`의 특산 아이템 전용 effect formatter만 fixed-position으로 수정했다. 제보된 육도 **`지력+%d 야망=%d`**, 수전 **`용맹+%d`** 외에 같은 route를 전수 감사해 신도 **`육지+%d`**, 태현생부 **`수명=%d단계`**, 오령수의 혼 **`전쟁 중에 사용하면…`**까지 5종을 확정했다. 목록/상세 복제 풀의 actual offsets는 list `0x1436C4/0x1436D4/0x1436DC/0x1436E4/0x143700`, detail `0x1967E4/0x1967F4/0x1967FC/0x196804/0x196820`로 총 10개 C-string이다. 육도/수전/신도/오령수의 혼은 원 visible length와 exact-fit, 태현생부는 13B→11B이며 같은 allocation 안에서 NUL/padding을 유지한다. `fix_data.bin`의 해당 아이템 effect ID도 45/47/55/56/58로 재검증했다. v256 대비 game diff는 `ExeFS/code.bin` 1개뿐이고 SHA-256=`BCF23935EA7B2804D979160861C00761748251D8ACD353D4D14F4CD7BB4695D2`; 비-code 83파일은 v256 byte-exact다. relocation/code growth/font/Message/Scenario/PNG/G1T 수정 없음. builder/독립 verifier/deterministic check/재검증 PASS. 이후 사용자가 Citra 실화면 확인 완료를 보고해 runtime PASS로 승격했다. 확인 기록=`analysis\v257_issue232_citra_runtime_confirmation.json`. Rebuild·`0004000000174D00`·Dummy update·Backup·패키징은 미수정. report=`analysis\v257_issue232_special_item_effects_report.json`.
- **직전 Patch 최종 기준본: v256-intermediate (`title_up_002.png` + `ktlogo_000.png` 이미지 갱신, 정적 검증 완료·Citra 확인 pending)** — exact v255 Patch 84파일을 `analysis\v256_image_update_baseline\PatchSnapshot`에 봉인하고, 명시된 두 PNG만 최신 G1T에 1:1 재삽입했다. `ktlogo_000.png`(512×256 RGBA)는 `RomFS/Opening/ktlogo.g1t` index 0, `title_up_002.png`(64×16 RGBA)는 `RomFS/StartMenu/title_up.g1t` index 2에 넣었다. 두 readback은 원 PNG와 pixel-exact이며 G1T header, `ktlogo` index 1, 공식 v1.1 `title_up` index 0/1 및 3-texture 구조는 v255 byte-exact다. v255 대비 game diff는 두 G1T뿐이고 SHA-256은 `ktlogo.g1t=4B3C049C4AC5E0E9EACA73096E5F289AA92F24238DE87B754EE5757533FAEA7C`, `title_up.g1t=251D4B56C91FAB8DDBAAC69EC32E8E55AA83F4ADC9062B9365D6CDC99773CBDE`; 나머지 82파일은 v255 byte-exact다. 이미지 생성 없음, builder/독립 verifier/deterministic check PASS. Rebuild·`0004000000174D00`·Dummy update·Backup·패키징은 미수정. report=`analysis\v256_image_update_report.json`.
- **직전 Patch 최종 기준본: v255-intermediate (Issue #230/#231 선인 대사 개선, 정적 검증 완료·Citra 확인 pending)** — exact v254 Patch 84파일을 `analysis\v255_issue230_231_xian_dialogue_baseline\PatchSnapshot`에 봉인했다. 두 건 모두 자허상인/선인 23-pointer fragment table의 동적 무장명 + suffix 조립형이다. #230 actual owner는 C347 / current `code.bin 0x1D2DEC`, table word[3] `0x1E5DE8=0x002D2DEC`; allocation은 다음 live #228 helper `0x1D2E08="에게서 "` 직전까지 28B이며 **`님의 명운,\n이제 다할 때가..`**가 27B visible + NUL = 28/28B exact-fit이다. #231 actual owner는 C363 / current `0x1D2FDC`, table word[2] `0x1E5DE4=0x002D2FDC`; 다음 live `각지` 문자열 `0x1D2FF8` 직전까지 28B이며 **`님께 죽을 상이..\n애석하오..`**가 27B visible + NUL = 28/28B exact-fit이다. `애`는 stale 보조값 `98B3`가 아니라 current v171+ effective alias **`9078`**이 권위다. v254 대비 game diff는 `ExeFS/code.bin` 1개뿐, SHA-256=`F843DA55D8B701BF1B0D773100508A974353EAD1889729BFB279D965DFA5BF8F`; 나머지 83파일 및 font/PNG/G1T는 v254 byte-exact다. pointer relocation/code growth 없음. builder/독립 verifier/deterministic check/common dialogue review **v255 authority 2280행 PASS**. report=`analysis\v255_issue230_231_xian_dialogue_report.json`.
- **직전 Patch 최종 기준본: v254-intermediate (Issue #228/#229 자허상인 조사 + 금 수입 띄어쓰기, 정적 검증 완료·Citra 확인 pending)** — exact v253 Patch 84파일을 `analysis\v254_issue228_229_text_cleanup_baseline\PatchSnapshot`에 봉인했다. #228 actual owner는 자허상인 23-pointer 조립형의 local connector이며 current table `code.bin 0x1E5DDC` word[12] (`0x1E5E0C`)가 `0x002D2E08`을 가리킨다. file offset `0x1D2E08`의 **`에게 `를 `에게서 `+NUL = 8/8B exact-fit**으로 바꾸고 C362 `0x1D2FCC="을(를)\n구하시오"`와 global connector는 보존한다. 기대 출력은 **`좌자에게서 둔갑천서3권을(를)\n구하시오`**다. #229 actual owner는 `RomFS/Message/msgsec10.dat` msgsec10_0017_000 / `word[17] WORD=0x043F -> 0x087E`; 34B span에서 **`상업을 올려 금수입을 늘립니다.` → `상업을 올려 금 수입을 늘립니다.`**로 바꾸고 기존 tail padding 2B 중 1B를 소비해 padding 1B를 남겼다. header/file size/`05 05 05` separator topology는 보존한다. v253 대비 game diff는 `ExeFS/code.bin`, `RomFS/Message/msgsec10.dat` 2개뿐이며 SHA-256=`code E28774311F3A33CF21A9D13C5271A52DDAEEC23A4EB3526AEAD5C4C4EA28A990`, `msg10 62ABBF69F6BB6235CF3D3A170CAF5F7A4897E82F844F3A0A041C6E8165982ECB`. builder/독립 verifier/deterministic check/common dialogue review 2280행 모두 v254 authority로 PASS. PNG/G1T/font, Original/Rebuild/Backup/Dummy update는 미수정. report=`analysis\v254_issue228_229_text_cleanup_report.json`.
- **직전 Patch 최종 기준본: v253-intermediate (Issue #227 장수 불만 문구 개선, 정적 검증 완료·Citra 확인 pending)** — exact v252 Patch 84파일을 `analysis\v253_issue227_officer_discontent_baseline\PatchSnapshot`에 봉인했다. runtime 조립은 `도시명 + local "의" + 동적 장수명 + C040 suffix`이며 official-update current prefix는 `code.bin 0x15A51C`, suffix는 **C040 / historical `0x15A210` → current `0x15A520`**다. prefix 4B 전용 슬롯은 `의\0+pad`를 **`의 `+NUL**로 exact-fit한다. C040은 Original visible 25B보다 실제 allocation이 넓으며 다음 ARM 함수 `0x15A53C` 직전까지 `0x15A520..0x15A53B` **28B 독립 문자열 영역**으로 증명됐다. 사용자 1안 **`<CITY>의 <OFFICER>은/는\n불만을 품은 듯합니다`**를 적용했고 suffix는 26B visible + NUL + 1B zero guard라 쉼표 fallback은 불필요하다. prefix/suffix ADR, shared assembly buffer literal, concat 호출, code size는 보존하며 pointer relocation 없음. v252 대비 game diff는 `ExeFS/code.bin` 1개뿐, SHA-256=`5AFEDDA0CBEA4D4EC8491D3144117BC73893525B8322EEB0A70680D24905CFA7`; 비-code 83파일과 font/PNG/G1T는 v252 byte-exact다. builder/독립 verifier/deterministic check PASS, common dialogue review는 v253 authority **2280행 PASS**이며 C040은 26/27B·여유1B로 표시된다. report=`analysis\v253_issue227_officer_discontent_report.json`.
- **직전 Patch 최종 기준본: v252-intermediate (Issue #224 매복 문구 4안, 정적 검증 완료·Citra 확인 pending)** — exact v251 Patch 84파일을 `analysis\v252_issue224_ambush_one_line_baseline\PatchSnapshot`에 봉인했다. actual owner는 `RomFS/Message/msgsec09.dat` **M09-D016 / mixed header word[17] BYTE=0x0217**, fixed 20B span `0x0217..0x022A`, separator `0x022B..0x022D`, 다음 direct `word[18]=0x022E`다. 사용자 확정 4안 **`<NAME1>군 <NAME2>,매복이었다`**를 적용했으며 기존 줄바꿈을 쉼표로 바꾸는 **`0x0220: 0A→2C` 단 1바이트 수정**이다. 88-word header, word[17]/[18], 모든 `05 05 05` separator, file size 2845B를 보존했고 parent-WORD target landing은 0건이다. 군주3자/장수4자 가정의 정적 폭은 **14.0/15.0 units**다. v251 대비 game diff는 `msgsec09.dat` 1개뿐, SHA-256=`63CE1A0F5BC296003AAD1848799DB6603AD9B4DFFA07400AD2C639FF94F8E377`; 나머지 83파일은 v251 byte-exact다. builder/독립 verifier/deterministic check/common review 2280행/battle review 520행 모두 v252 authority로 PASS. report=`analysis\v252_issue224_ambush_one_line_report.json`.
